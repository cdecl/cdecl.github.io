...
변수가 설정되지 않았거나 빈 문자열일 때 대체값을 지정하는 표준 문법입니다.

```bash
PORT="${CLAUDE_PICK_PORT:-8082}"
```

`CLAUDE_PICK_PORT`가 없거나 비어있으면 `8082`를 사용합니다.
...
- 변수에 값이 있으면 그 값을 그대로 반환
- 변수가 **미설정(unset)** 이거나 **빈 문자열(`""`)** 이면 기본값 반환

### `:` 유무에 따른 차이

| 문법 | 미설정(unset)일 때 | 빈 문자열(`""`)일 때 |
|------|-------------------|---------------------|
| `${VAR:-8082}` | `8082` 사용 | `8082` 사용 ✅ (권장) |
| `${VAR-8082}` | `8082` 사용 | 빈 문자열(`""`) 그대로 사용 |

콜론(`:`)을 포함한 `:-` 형식이 unset과 빈 문자열 두 경우를 모두 처리하므로 일반적으로 권장됩니다.
